...
func Parse(cronExpr string) (*Schedule, error) {
	...
	schedule := &Schedule{
		Second:  fieldFunc(fields[0], seconds),
		Minute:  fieldFunc(fields[1], minutes),
		Hour:    fieldFunc(fields[2], hours),
		Day:     fieldFunc(fields[3], dom),
		Month:   fieldFunc(fields[4], months),
		WeekDay: fieldFunc(fields[5], dow),
	}
	if err != nil {
		return nil, err
	}
